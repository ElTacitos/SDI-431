<template>
  <Product v-for="(product, index) in products" :key="index" :attributes="product.attributes"/>
  <Button @click="fetch">Fetch</Button>
</template>

<script setup lang="ts">const products = ref([])
  const tok = ""
  async function fetch(): Promise<void> {
    const {data} = await useFetch('http://localhost:1337/api/products', {headers: {'Authorization': 'Bearer ' + tok,  'Content-Type': 'application/json'}});
    console.log(data)
    const value = data['_rawValue'].data;
    console.log(value)
    products.value = value;
  }
</script>

<style scoped>

</style>
